...
포인터에 배열명을 대입하여 포인터 연산을 이용한다.

예를 들어
int arr[3] 배열이 선언되고 int* p 라는 포인터 변수가 있을 때 p = arr; 로 대입하면 p는 배열의 첫 번째 원소 주소를 가리킨다.
p + i 는 arr[i]의 주소가 된다.
### 포인터를 이용하여 배열 각 원소의 값을 구하는 방법을 설명하라
위에서처럼 포인터에 배열명을 대입한 후, 역참조를 이용하여 값을 구할 수 있다.
예를 들어
int arr[3] = {10, 20, 30};일 때 *p 는 arr[0]의 값이 된다 =10
*(p + i) 는 arr[i]의 값이다.
